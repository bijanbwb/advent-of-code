module AdventOfCode.Year2015.Day10 exposing
    ( rawInput
    , run
    )
...
-- Given an input, run n times:
--   Split input into groups of repeating numbers.
--   For each group of numbers.
--     Take the num of digits in that group.
--     And the digit that makes up that group.
--     Create a string with those results.
-- INPUT
